...
- Claude Code 侧的**元数据**取自官方 `session-meta`，**对话正文**取自
  `~/.claude/projects` 的 jsonl（两边按 `session_id` 关联，实测 393/394 命中）。
  正文只用来打标，**不参与任何计数**——所有数字仍来自官方字段。
  如果那个目录被清理过，该会话就只剩元数据，深度分析会跳过它（`adi doctor` 会报 `E_NO_TRANSCRIPT`）。
- 单个会话最多取 400 行正文，**超出时保首尾、省略中段**。极长会话的中间过程不会被覆盖，
  但开头的诉求和结尾的验收一定在。
