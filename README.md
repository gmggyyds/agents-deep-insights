...
| **你在要求 AI 做什么** | 每条消息按「在要求什么」分三类计数（派活 / 想清楚 / 把关），并与结果、摩擦交叉，看有没有关系 |
...
**协作模式那一段是描述，不是打分。** 它统计的是你的消息在要求什么——已经知道要什么让它去做
（派活）、还没想清一起收敛（想清楚）、还是对产出把关和定规则（把关）。三者相加会超过 100%，
因为一条消息常常同时要求好几件事，**必须多标签计数**：实测单标签取主导意图会把「想清楚」
低估 2.5 倍（8.2% vs 20.5%）。

同样重要的是它**不能**用来做什么：占比高低不代表水平高低。实测同一个人相隔三个月，
三类占比从 6.3/75.8/17.9 变成 7.2/78.6/14.2，主要由那段时间在干什么类型的活决定。
与结果的交叉表是**相关性不是因果**（简单任务既不需要「想清楚」又天然容易成功，
这一条就足以把关系拉成反向），任一组少于 5 个会话时报告直接标「样本不足」不给百分比。

**如果你是在课程或分享里第一次听到这三个词**，它们和课上讲的「AI Thinking / AI Working /
Oversight」是同一套东西，报告里换成了更直白的说法：

| 报告里写的 | 课上讲的 | 判据 |
|---|---|---|
| 派活 `delegate` | **Working** | 已经知道要什么，让它去产出或执行 |
| 想清楚 `deliberate` | **Thinking** | 还没想清，和它一起收敛判断 |
| 把关 `steer` | **Oversight** | 对已有产出或未来行为把关、定规则、留下否决理由 |

**最容易读错的一点：这个百分比按「消息条数」算，不按你花的时间或心力算。** 一条「帮我想清楚
这个模式该不该做」你可能想了半小时，一条「继续」只要一秒钟——在这份统计里它们都算 1 条。
所以**条数占比天然低估「想清楚」的实际投入**：看到「想清楚 7%」不等于「我只有 7% 的精力在思考」。

如果你听过某种说法是「我现在九成时间在想、一成在干」，那描述的是**心力重心**，和这里的
**条数占比**不是同一个口径，**两个数不能直接比大小**。真要看自己有没有变化，比的是同一个人、
同一个口径、不同时间的两次统计——而不是拿自己的条数占比去对别人的心力描述。
...
- 「你在要求 AI 做什么」这一段依赖对话正文，因此**同样只对 Codex 会话有效**；
  Claude Code 侧拿不到 transcript，这一段不会出现。
