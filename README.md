...
（派活）、还没想清一起收敛（想清楚）、还是对产出把关和定规则（把关）。一条消息常常同时
要求好几件事，会**同时计入多类**：实测单标签取主导意图会把「想清楚」低估 2.5 倍
（8.2% vs 20.5%）。报告给的百分比是**三类标签各占多少**（合计 100%），另附每类出现在
多少个会话里——两个数回答的是不同问题：注意力怎么分配，以及这类要求涉及多少次协作。
...
这一条就足以把关系拉成反向）。分组方式是按每个会话里该模式**所占比例**切成高低两半
（不是「有/无」——多标签下主导模式几乎每个会话都有，用有无分组会让它永远出不了结论）；
任一侧少于 5 个会话、或占比几乎没有差异时，报告直接说明并不给百分比。
