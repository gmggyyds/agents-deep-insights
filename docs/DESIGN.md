...
L5 合成    叙事生成（第二次 LLM 调用，与 L3 性质不同）
```

**不可动摇的分界**：L1/L2/L4 零 LLM。所有计数、排序、门槛判定由确定性代码完成。

⚠️ 这条原则约束的是**计数**，不约束**叙事**。v0.1 曾把它执行过头——只实现了 L3，
L5 退化成把数字填进模板，产出因此只是一张统计报表：叙事字符数 49（还是免责声明），
对照官方 `/insights` 的 31,841。原料其实已经采到（`friction_detail` / `brief_summary` /
`user_instructions` 三个自由文本字段），只是从未使用。

L3 与 L5 是**两次性质不同的调用**：

| | L3 打标 | L5 合成 |
|---|---|---|
| 输入 | 单个会话的转录 | L4 算好的数字 + L3 采集的自由文本 |
| 输出 | 受控枚举，喂给统计 | 人话叙事，直接给人读 |
| 约束 | 枚举闭合 | 不得重新计数或推翻排序 |

L5 的报告结构（5 段，比官方 8 段聚焦）：一句话结论 / 你做得好的 /
摩擦按归因分三段讲 / 可直接粘贴的规则（只从 `user_actionable` 且重复 ≥3 的候选出）/
下一步可试（带可粘贴提示词）。第三、四段是官方没有的——它把 12 类摩擦混报，
读者无法区分哪些是自己能改的。
...
### 9.4 调用 codex exec 必须隔离执行环境（三个参数缺一不可）

```js
cwd: <临时目录>           // 不能留在项目里
'--ignore-user-config'    // 切断用户级配置
maxBuffer: 64MB           // 默认 1MB 会 ENOBUFS
```

三者是同一个问题的三面。在仓库目录里调 `codex exec`，它会加载项目的 `AGENTS.md`
和全局 skill 配置，于是把「读输入、出 JSON」理解成「有活要干」——实测它去读
skill 文档、找 TODO 文件、用 `rg` 扫了整个 home 目录，输出几百 KB，
撑爆 Node `execFileSync` 默认 1MB 缓冲。

而 `ENOBUFS` 的报错信息里**完全看不出真正原因**（只说缓冲区不足），
真正的线索埋在几百 KB 的 stderr 里。修好之后：一次成功不再需要重试，
单次调用耗时 79s → 32s。
