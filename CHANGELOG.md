...
报告新增一段展示三类标签占比（合计 100%，另附每类出现在多少个会话里），
并与 `outcome` / `friction` 交叉。
...
- **交叉表是相关性不是因果**，且被任务难度混杂（简单任务既不需要 deliberate 又天然容易成功）。
  分组按会话内该模式**所占比例**切高低两半——不能用「有/无」，多标签下主导模式几乎
  每个会话都出现，对照组恒空，那一行会永远停在「样本不足」。
  任一侧少于 5 个会话、或占比无差异切不开时直接说明，不给百分比。
  成功率分母只用可判定集，结果说不清的单列，不塞进任何一边。
