...
### 新增：Claude Code 会话支持深度分析（`adi run`）

此前 `adi run` 只能分析 Codex 会话——官方 `session-meta` 只有元数据、没有对话正文，
而 L3 打标离了正文什么也做不了。现在正文从 `~/.claude/projects` 的 jsonl 读取，
与官方 meta 按 `session_id` 关联（实测 394 个 session-meta 命中 393 个，99.7%；
未命中的那个会话正文里确实没有人说过话）。

🔴 **划了一条硬线：jsonl 只用来取文本，绝不用来算任何数字。**
token、工具调用数、失败数、提交数一律沿用官方字段。
「自建 jsonl 解析器漏算 cache token 500 倍」那次事故的根因是**自建计数**，
不是自建读取；两件事不能混为一谈，也不该因为噎住就不吃饭。

判「哪些 user 消息是人打的字」用**结构化标记**，不猜文本：
`isSidechain`（子代理任务书）/ `isMeta`（图片占位、skill 全文、命令展开）/
`isCompactSummary`（上下文压缩摘要，助手写的、含历史 prompt 原文，
不滤会让同一批内容在多个会话里重复计入）。文本前缀表保留作旧格式兜底。

其它实测修正：
- `content` 有三种形态（`string` / `[{text}]` / `[{image},{text}]`），
  只认 string 会漏掉「贴图 + 提问」那一类，而那类信息量最高
- 索引必须**递归**：30,307 个 jsonl 里 15,123 个在更深层目录，只扫两层漏一半
- 长度下限从 `< 4` 改为 `< 2`：中文 3 个字就是完整指令（「干这个」「改一下」），
  按英文习惯设阈值会把它们整批丢掉
- 单会话行数上限 400 行，**超出时保首尾**、中段省略。不能只取前 400 行：
  全量只有 6.1% 的会话撞上限，但采样器按 `userMessages` 降序挑，
  默认 `--limit 30` 下**被送进模型的样本有 63% 撞上限**，丢的全是尾部——
  而结尾正是验收边界（「这样就行了」／「还是不对」）
- 同一条 assistant 消息里的多个 `tool_use` 合并成一行：实测一个 2977 行的会话里
  `[tool]` 独占 358 行（89.5%），把行预算吃光，最后只有 8 条用户消息进模型
- 单条裁剪复用 `budget.mjs` 的 `clipHeadTail`，不再自抄第四份实现
- `transcriptComplete` 按是否触发截断如实赋值（此前只要读到一行就报 true）
- `doctor` 跟着扩面：新增 `claudeTranscripts` 数据源、`claudeTranscriptFiles` /
  `claudeMetaWithTranscript` 计数，以及「有元数据但一个正文都找不到」时的
  `E_NO_TRANSCRIPT`。新增数据源却不扩自检，等于让真故障躲在一片绿灯里
